...
import { ClipboardList, RefreshCw, AlertTriangle, FileSpreadsheet, CalendarDays } from "lucide-react";
...
export default function RequestErrorBoundary({ error, reset }: ErrorProps) {
  const { lang } = useI18n();
  const isTh = lang === "th";

  useEffect(() => {
    console.error("Leave Request Error Boundary caught an error:", error);
  }, [error]);
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-[60vh] px-4 w-full">
      ...
      >
        ...
        {/* Request Context Visual Icons */}
        <div className="flex justify-center items-center gap-2 mb-6">
          <div className="w-12 h-12 bg-violet-50 dark:bg-violet-950/30 text-violet-500 rounded-2xl flex items-center justify-center shadow-inner relative">
        ...
        <h2 className="text-xl font-bold text-slate-900 dark:text-white tracking-tight mb-2">
          {isTh ? "เกิดข้อผิดพลาดในระบบใบลา" : "Leave Request System Error"}
        </h2>
        
        <p className="text-sm text-slate-500 dark:text-slate-400 leading-relaxed mb-6 max-w-xs mx-auto">
          {isTh
            ? "ไม่สามารถโหลดแบบฟอร์มขอลาหยุด หรือประมวลผลส่งใบลาได้สำเร็จ กรุณาตรวจสอบข้อมูลวันลาคงเหลือของคุณ หรือติดต่อฝ่ายบุคคลเพื่อแก้ไขปัญหา"
            : "We couldn't load the leave form or submit your request. Please check your remaining leave quota or contact HR support."}
        </p>
        ...
        <div className="flex justify-center">
          <button
            onClick={() => reset()}
            className="w-full flex items-center justify-center gap-2 px-5 py-3 rounded-2xl bg-gradient-to-r from-violet-500 to-indigo-600 hover:from-violet-600 hover:to-indigo-700 text-white font-bold text-sm shadow-lg shadow-violet-500/25 hover:shadow-xl hover:shadow-violet-500/35 hover:scale-[1.02] active:scale-[0.98] transition-all duration-200 cursor-pointer border-none"
          >
